...
3. `PRAGMA wal_checkpoint(TRUNCATE)` run before schema creation
4. Schema creation using `schema::SCHEMA_SQL`
...
pub async fn open_db(path: &str) -> Result<Connection, tokio_rusqlite::Error> {
    ...
    // Step 2: Checkpoint any leftover WAL from a previous run, then apply schema.
    conn.call(|db| {
...
`grep "thread_id" airev-core/src/schema.rs` confirms the multi-round thread column exists in comments.
  ...
  <done>
airev-core compiles. The Session, Comment, Hunk, DiffLine types are pub-exported from airev-core::types. open_db() in airev-core::db applies the full WAL pragma sequence. The sessions, threads, and comments tables are defined with STRICT and the multi-round schema shape (thread_id, round_number, status enum).
  </done>
</task>
...
<success_criteria>
The workspace compiles. Three crates exist with correct dependency isolation. airev-core exports shared types and an open_db() function that implements the full WAL pragma sequence and creates the multi-round SQLite schema. cargo build --workspace exits 0.
</success_criteria>
