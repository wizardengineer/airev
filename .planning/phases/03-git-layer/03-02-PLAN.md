...
must_haves:
  truths:
    ...
    - "Word-level (inline) diff is applied to consecutive -/+ line pairs via similar::TextDiff::from_words(); changed words rendered bold"
  artifacts:
    - path: "airev/src/git/worker.rs"
      provides: "git_worker_loop() function owning Repository, handling all 4 diff modes, syntax-highlighting output, word-level diff via similar"
...
<task type="auto">
  <name>Task 3: Add word-level (inline) diff spans to highlight_hunks</name>
  <files>airev/src/git/worker.rs</files>
  <action>
    Extend `highlight_hunks` in `airev/src/git/worker.rs` to apply word-level diff emphasis for
    consecutive removed/added line pairs within each hunk. This is the "within-line diff" feature
    using `similar 2.7` with the `inline` feature (already added as a workspace dependency in Plan 01).

    **Where to add:** Inside `highlight_hunks`, after constructing the syntect spans for each line
    and before pushing the final `Line` to `highlighted_lines`. For each hunk, pair consecutive
    `-` (removed) lines with the immediately following `+` (added) line, then apply word-level
    diff spans on top of the base diff color.

    **Add the following private helper function** (≤50 lines) to `worker.rs`:

    ```rust
    /// Computes word-level diff spans for a removed/added line pair.
    ///
    /// Returns two parallel `Vec<Span<'static>>`: one for the old line (removed words
    /// emphasized with a dim background), one for the new line (added words emphasized
    /// with a bright background). Words that changed are marked with `emphasized = true`
    /// from `iter_strings_lossy`.
    ///
    /// Uses `similar::TextDiff::from_words` — requires the `inline` feature on similar 2.7.
    fn word_diff_spans(
        old_line: &str,
        new_line: &str,
                    ...
                    match change.tag() {
                        ChangeTag::Delete => {
                            let style = if emphasized {
                                // Emphasized removed word: red fg + bold to mark the exact change.
                        ...
                        ChangeTag::Insert => {
                            let style = if emphasized {
                                // Emphasized added word: green fg + bold to mark the exact change.
                        ...
                        ChangeTag::Equal => {
                            // Unchanged words appear in both lines without emphasis.
                            let span = Span::styled(text.clone(), Style::default().fg(Color::DarkGray));
    ...
    **Integrate into `highlight_hunks`:** Replace the current per-line push logic with a
    two-pass approach that detects consecutive `-`/`+` pairs:

    ```rust
    // Inside the per-hunk line loop, instead of always pushing Line::from(spans) immediately:

    // Buffer removed lines waiting for a potential paired added line.
    ...
    for dl in &hunk.lines {
        ...
        // Strip the origin prefix character for syntect highlighting.
        let code = if content.starts_with(['+', '-', ' ']) { &content[1..] } else { content };
        let base_spans = build_syntect_spans(code, &mut h, &PS);  // existing syntect logic

        match origin {
            '-' => {
                // Flush any pending removed line without a pair (it had no following '+').
                if let Some((_, spans)) = pending_removed.take() {
                    highlighted_lines.push(ratatui::text::Line::from(spans));
                }
                // Buffer this removed line with its base spans.
                pending_removed = Some((code.to_owned(), {
            ...
            '+' => {
                if let Some((old_code, _old_base_spans)) = pending_removed.take() {
                    // Pair found: apply word-level diff to both lines.
                    let (old_word_spans, new_word_spans) = word_diff_spans(&old_code, code);
                    let mut old_line_spans = vec![Span::styled("- ", Style::default().fg(Color::Red))];
                    old_line_spans.extend(old_word_spans);
                    highlighted_lines.push(ratatui::text::Line::from(old_line_spans));
                    let mut new_line_spans = vec![Span::styled("+ ", Style::default().fg(Color::Green))];
                    new_line_spans.extend(new_word_spans);
                    highlighted_lines.push(ratatui::text::Line::from(new_line_spans));
                } else {
                    // No preceding '-' line: pure addition, use base spans.
                    let mut s = vec![Span::styled("+ ", Style::default().fg(Color::Green))];
                    s.extend(base_spans);
                    highlighted_lines.push(ratatui::text::Line::from(s));
                }
            }
            _ => {
                // Context line or hunk header: flush pending removed if any.
                if let Some((_, spans)) = pending_removed.take() {
                    highlighted_lines.push(ratatui::text::Line::from(spans));
                }
                let mut s = vec![Span::styled(format!("{} ", origin), Style::default().fg(Color::DarkGray))];
                s.extend(base_spans);
                highlighted_lines.push(ratatui::text::Line::from(s));
            }
        }
    ...
    **Pairing heuristic:** Match each `-` line with the very next `+` line in the same hunk.
    If a `-` is followed by another `-` (not a `+`), flush it as unpaired before buffering the
    new `-`. This matches the behavior of `git diff --word-diff` and is sufficient per the
    RESEARCH.md open question recommendation.

    **Important:** Extract the syntect span-building logic from `highlight_hunks` into a small
    private `build_syntect_spans(code: &str, h: &mut HighlightLines, ps: &SyntaxSet) -> Vec<Span<'static>>`
    helper so it can be called for both the buffered and immediate paths without duplication.
    This helper should be ≤15 lines.

    **Add import** at the top of `worker.rs` (already present from Plan 01, but confirm):
    ```rust
    use similar::{ChangeTag, TextDiff};
    ```

    After implementation, run `cargo build -p airev`. Fix any:
    - `similar` not in scope → confirm `similar = { workspace = true }` is in `airev/Cargo.toml`.
    - Borrow checker issues with `pending_removed` → use `.take()` to move out of the `Option`.
    - `Span::styled` type mismatch → `Span<'static>` requires owned `String`, not `&str`.
      Use `.to_owned()` on `value` from `iter_strings_lossy`.
    - Function line-count violations → keep `word_diff_spans` and `build_syntect_spans` as
      separate private functions; `highlight_hunks` itself must stay ≤50 lines (delegate to them).
  </action>
  <verify>
    `cargo build -p airev` exits 0.
    `grep "TextDiff::from_words\|word_diff_spans\|iter_inline_changes" airev/src/git/worker.rs`
    confirms `similar` integration is present.
    `grep "similar" airev/src/git/worker.rs` shows the import.
    Manual spot-check: In a repo with a modified file (edit a line), run `cargo run -p airev` —
    the diff panel should show the removed and added versions of the edited line with the
    changed words rendered in bold (more emphatic than unchanged words on the same line).
  </verify>
  <done>
    highlight_hunks applies word-level diff via similar::TextDiff::from_words() for consecutive
    -/+ line pairs within each hunk. Changed words are rendered bold; unchanged words on changed
    lines are rendered in the standard diff color (red/green). Unpaired removed or added lines
    render normally without word-level emphasis. cargo build exits 0.
  </done>
</task>
...
`grep "TextDiff::from_words" airev/src/git/worker.rs` confirms word-level diff is implemented.
</verification>

<success_criteria>
- git/worker.rs implements git_worker_loop with all 4 diff modes and syntect highlighting
- git/worker.rs applies word-level diff via similar::TextDiff::from_words() for consecutive -/+ pairs
